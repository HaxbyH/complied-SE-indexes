...
void index(const char* input) {
    ...
    for (std::string primary_key :  keys) {
        ...
        pointer = postings_file.tellp();
        std::sort(single_postings.begin(), single_postings.end(), p_comparison);

        int32_t neg = -1;
        int32_t current_bm = 256;
        int num_bm_25_hits = 0;
        for (int i = 0; i < single_postings.size(); i++) {
            if (single_postings[i].second < current_bm) {
                current_bm = single_postings[i].second;
                num_bm_25_hits += 1;
                postings_file.write(reinterpret_cast<const char *>(&neg), sizeof(neg));
                postings_file.write(reinterpret_cast<const char *>(&current_bm), sizeof(current_bm));
            }
            postings_file.write(reinterpret_cast<const char *>(&single_postings[i].first), sizeof(single_postings[i].first));
        }
        int size = single_postings.size()*sizeof(int32_t) + num_bm_25_hits*2*sizeof(int32_t);
