...
#include "disks1and2/includes.h"
#include "disks1and2/functions.h"
#include "disks1and2/vocab.h"
...
int search(char** words, int numWords) {
    ...
    qsort(rsv_pointers, documents_in_collection, sizeof(*rsv_pointers), compare_rsv);

    for (int i = 0; *rsv_pointers[i] != 0 && i < documents_in_collection && i < 15; i++) {
        // std::cout << rsv_pointers[i]-rsv_scores << std::endl;
        // std::cout << *rsv_pointers[i] << std::endl;
        std::cout << doc_array[rsv_pointers[i]-rsv_scores] << " " << *rsv_pointers[i] << std::endl;
    }
