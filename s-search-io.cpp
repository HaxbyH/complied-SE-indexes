...
typedef struct
	{
	const char *term;
	const int *postings_list;
	uint32_t postings_list_length;
	} dictionary;

#include "indexs/wsj.small.h"
...
void search(char** words, int numWords) {
    ...
    for (int i = 0; i < numWords; i++) {
        ...
        if (got != NULL) {
            ...
            // std::cout << p_length << std::endl;
            // return;

            // go through all postings
            int current_bm = 256;
            int hit_change = false;
            for (int i = 0; i < p_length; i++) {
                if (got->postings_list[i] == -1) {
                    hit_change = true;
                } else if (hit_change) {
                    current_bm = got->postings_list[i];
                    hit_change = false;
                } else {
                    int d = got->postings_list[i];
