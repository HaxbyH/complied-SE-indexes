...
// File name
const char* INDEX_FILE_NAME = "./testdoco/";
...
void index(const char* input) {
    ...
    for (std::string primary_key : keys) {
        ...
        for (int i = 0; i < single_postings.size(); i++) {
            ...
            // int current_impact = single_postings
            // add impact score function to unique term function
            // function_file << "\t" << primary_key << "_" << single_postings[i].second << "();\n";
            std::cout << single_postings[i].first << " " << single_postings[i].second << std::endl;
        }
        postings_file << "}\n";
