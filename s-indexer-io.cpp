...
std::unordered_map<std::string, int> postings_size;
...
void index(const char* input) {
    ...
    for (std::string primary_key : keys) {
        ...
        outfile << "const int i_" << primary_key << "[] = {";
        ...
        for (int i = 0; i < single_postings.size(); i++) {
            if (single_postings[i].second < current_bm) {
                ...
                num_bm_25_hits += 1;
                outfile << "-1, " << current_bm << ", ";
            }
            outfile << single_postings[i].first << ", ";
        }
        postings_size[primary_key] = num_bm_25_hits;
        outfile << "};\n";
    ...
    for (int i = 0; i < keys.size(); i++) {
        outfile << "\t{\"" << keys[i] << "\", i_" << keys[i] << ", " << vocabulary[keys[i]].size() + (postings_size[keys[i]]*2) << "},\n";
    }
