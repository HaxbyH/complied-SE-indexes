...
// File name
const char* INDEX_FILE_NAME = "./indexs/testdoco/";
...
void index(const char* input) {
    ...
    int md = mkdir(INDEX_FILE_NAME, 0777);
    ...
    for (std::string primary_key : keys) {
        ...
        for (int i = 0; i < single_postings.size(); i++) {
            function_file << "\trsv_scores[" << single_postings[i].first << "] += " << single_postings[i].second << ";" << std::endl;
            function_file << "\taccValid(&rsv_scores[" << single_postings[i].first << "]);\n";
        }
